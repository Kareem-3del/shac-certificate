...
export class SubscriptionsService {
  ...
  async getActiveSubscriptions(): Promise<number> {
    ...
    // Count users with active subscriptions from user_subscriptions_subscription table
    const result = await this.subscriptionRepository.query(`
      SELECT COALESCE(COUNT(DISTINCT userId), 0) as count 
      FROM user_subscriptions_subscription 
    `);
  ...
  async getTotalSubscriptionRevenue(): Promise<number> {
    // Get total lifetime revenue from all completed subscription transactions
    const result = await this.subscriptionRepository.query(`
      SELECT COALESCE(SUM(amount), 0) as total 
      FROM transactions 
