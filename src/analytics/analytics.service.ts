...
export class AnalyticsService {
  ...
  async getRevenueStats(days: number = 30) {
    ...
    const previousRevenue = await this.transactionsRepository
      .createQueryBuilder('transaction')
      .select('SUM(transaction.amount)', 'total')
      .where('transaction.created_at BETWEEN :previousStartDate AND :startDate', { 
        previousStartDate, 
        startDate 
      })
      .andWhere('transaction.status = :status', { status: 'completed' })
    ...
    const growth = previous > 0 ? parseFloat(((current - previous) / previous * 100).toFixed(1)) : 0.0;
  ...
  async getCertificateStats(days: number = 30) {
    ...
    const previousCerts = await this.certificatesRepository
      .createQueryBuilder('certificate')
      .where('certificate.created_at BETWEEN :previousStartDate AND :startDate', {
        previousStartDate,
        startDate
      })
      .getCount();
  ...
  async getAdvertisingROI(days: number = 30) {
    ...
    const prevROI = prevAdSpend > 0 ? ((prevTotalRevenue - prevAdSpend) / prevAdSpend * 100) : 0;
  ...
  async getCertificatesChartData(days: number = 30) {
    ...
    const data = await this.certificatesRepository
      .createQueryBuilder('certificate')
      .select([
        'DATE(certificate.created_at) as date',
        'COUNT(*) as count'
      ])
      .where('certificate.created_at > :startDate', { startDate })
  ...
  async getDashboardOverview() {
    const [revenue, totalRevenue, certificates, users, advertising] = await Promise.all([
  ...
  async getAdCampaignData(platform?: string) {
    ...
    return platform ? campaigns.filter(c => c.platform === platform) : campaigns;
  }
